...
% Calculate padding pixels needed
paddingRows = abs(w * sin(angle) + h * cos(angle) - h) / 2;
paddingCols = abs(w * cos(angle) + h * sin(angle) - w) / 2;

% Add padding to the image
img_padded = padarray(img_transfull, [ceil(paddingRows) ceil(paddingCols)], 128, 'both');

% Rotate the padded image slice using imrotate with 'bilinear' interpolation and 'loose' bounding box
img_rotated = imrotate(img_padded, 180-angleDeg, 'bilinear', 'loose');
