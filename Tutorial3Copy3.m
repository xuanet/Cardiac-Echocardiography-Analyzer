...
% Create a 3D rotation matrix based on the line made by user
switch dimension
    case 'W'
        rotationAxis = [0, y(2) - y(1), x(2) - x(1)];%[0, y(2) - y(1), x(2) - x(1)];
    case 'H'
        rotationAxis = [y(2) - y(1), 0, x(2) - x(1)];%[y(2) - y(1), 0, x(2) - x(1)];
    case 'D'
        rotationAxis = [y(2) - y(1), x(2) - x(1), 0];%[y(2) - y(1), x(2) - x(1), 0];
end

% Normalize the rotation axis to make sure it's a unit vector
rotationAxis = rotationAxis / norm(rotationAxis);

% Rotate the volume using imrotate3 with 'linear' interpolation and 'loose' bounding box
vol_rotated = imrotate3(vol_transfull, 180-angleDeg, rotationAxis, 'linear', 'loose');




% Extract the relevant slice from the rotated volume
switch dimension
    case 'W'
        extract_rotated = vol_rotated(centerW, :, :, 1);
        originalTitle = get(get(sp2_1, 'Title'), 'String');
    case 'H'
        extract_rotated = vol_rotated(:, centerH, :, 1);
        originalTitle = get(get(sp2_2, 'Title'), 'String');
    case 'D'
        extract_rotated = vol_rotated(:, :, centerD, 1);
        originalTitle = get(get(sp2_3, 'Title'), 'String');
end

img_rotated = squeeze(extract_rotated);
